...
const SimpleLineChart: React.FC<{ data: ChartDataPoint[]; dataKey: keyof ChartDataPoint; color: string; height?: number }> = ({ 
  ...
  const formatValue = (value: number) => {
    if (dataKey === 'price') return `$${value.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
    ...
    <div className="w-full" style={{ height }}>
      <div className="relative w-full h-full bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800 rounded-xl border border-border/50 p-6 shadow-sm">
        <svg className="w-full h-full" viewBox="0 0 500 200" preserveAspectRatio="none">
          ...
          {/* Horizontal reference lines with Y-axis labels */}
      ...
      <div className="relative w-full h-full bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800 rounded-xl border border-border/50 p-6 shadow-sm">
        ...
        <div className="flex items-end justify-between h-full space-x-1 pl-8 pr-2 pb-4">
          {data.map((d, index) => {
            ...
            const heightPercent = Math.max((value / maxValue) * 85, 3); // Increased base height
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  {/* Value indicator for high values */}
                  {isHighValue && (
...
export function TokenChart({ tokenSymbol }: TokenChartProps): JSX.Element {
  ...
  // Fetch real ETH data from Token Overview API
  useEffect(() => {
  ...
  // Generate chart data from real token data
  const generateChartData = (): ChartDataPoint[] => {
    ...
    const baseVolume = tokenData.volume_24h || 15000000000; // $15B default volume
    const baseLiquidity = tokenData.liquidity || 5000000000; // $5B default liquidity

    for (let i = 23; i >= 0; i--) {
      const time = new Date(now.getTime() - i * 60 * 60 * 1000);
      const hourlyVariation = (Math.random() - 0.5) * 0.1; // ±5% variation

      dataPoints.push({
        time: time.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' }),
        price: basePrice * (1 + hourlyVariation),
        volume: baseVolume / 24 * (1 + hourlyVariation * 2), // Hourly volume
